...
try {
  ...
  if (Number(migration.rows[0]?.count ?? 0) !== 4) throw new Error("recovery_migration_ledger_invalid");
  ...
  console.log(JSON.stringify({ event: "database.recovery_verified", migrationCount: 4, requiredTables: requiredTables.length }));
} finally {
  await pool.end();
}
